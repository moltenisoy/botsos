...
Versión adaptada para usar LM Studio como backend de LLM.
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_dir / "botsos_lmstudio.log", encoding='utf-8'),
...
def check_platform():
    ...
    if platform.system() != "Windows":
        print("⚠️ ADVERTENCIA: BotSOS-LMStudio está diseñado exclusivamente para Windows.")
...
def check_dependencies():
    ...
    # OpenAI library para LM Studio API
    try:
        import openai
    except ImportError:
        missing.append("openai (ejecute: pip install openai)")
...
def check_lmstudio():
    ...
    try:
        ...
        # Crear cliente con base URL de LM Studio
        ...
        if models.data:
            ...
            print("   Por favor cargue un modelo en LM Studio")
            return False
...
def check_system():
    ...
    print(f"\n{'=' * 60}")
    if all_ok and lmstudio_ok:
        print("  ✓ Sistema listo para ejecutar BotSOS-LMStudio")
    elif all_ok and not lmstudio_ok:
        print("  ⚠️ Sistema listo pero LM Studio no está disponible")
        print("     Inicie LM Studio y cargue un modelo antes de usar la aplicación")
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f"{__app_name__} - Administrador de Sesiones Multi-Modelo con LM Studio",
...
Ejemplos:
  ...
  python main.py --check-system  Verificar requisitos del sistema
    ...
    print(f"""
╔═══════════════════════════════════════════════════════════════╗
║   {__app_name__} v{__version__} - Administrador de Sesiones        ║
║   Backend: LM Studio (API compatible con OpenAI)              ║
║   Diseñado exclusivamente para Windows                        ║
╚═══════════════════════════════════════════════════════════════╝
    """)
